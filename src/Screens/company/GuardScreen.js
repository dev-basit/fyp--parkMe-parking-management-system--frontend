...
import { employeepParkingHistoryColumns } from "../../constants/data/parkings";
...
export function AllGuardsForCompanyOwner() {
  ...
  const fetchAllGuards = async () => {
    try {
      const response = await guardService.getGuards("?companyId=" + getLocalStorageItem("companyId"));
...
export function ParkingHistoryForGuard() {
  ...
  const call = async () => {
    let myDetails = auth.getCurrentUserDetails();
    console.log("currentUserdetails: ", myDetails);

    let guardDetails = await guardService.getGuards("?userId=" + myDetails._id);
    console.log("currentUserparkingdetails: ", guardDetails.data[0]);

    if (guardDetails?.data[0]?.assignedParkingId)
      fetchAllParkings(guardDetails?.data[0]?.assignedParkingId);
  };

  const fetchAllParkings = async (id) => {
    try {
      const response = await parkingService.getParkings(`?parkingAreaId=${id}`);
